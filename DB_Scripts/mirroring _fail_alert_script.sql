...
BEGIN 
   ...
   FROM @databases 
   SET @string = 'Host: '+@@servername+'- database_name--> '+CAST(DB_NAME(@DbId) AS VARCHAR)+ '-DB Mirroring is--> '+@state +' - notify DBA' 
   EXEC msdb.dbo.sp_send_dbmail 'Suraj_DBA', 'tatesuraj10@gmail.com', @body = @string, @subject = @string 
